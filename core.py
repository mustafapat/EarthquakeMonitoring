...
def print_earthquake_details(data: Dict[str, Any], local_time_str: str, delay_minutes: Optional[float]):
    ...
    # --- HATA DÜZELTME: mag_str ve depth_str burada tanımlanmalı ---
    # 'data' sözlüğünden ilgili değerleri al
    mag = data.get('mag')
    depth = data.get('depth')
    # Yazdırmak için formatla
    mag_str = f"M{mag:.1f}" if mag is not None else "M?"
    depth_str = f"{depth:.1f} km" if depth is not None else "? km"
    # -------------------------------------------------------------

    # Artık değişkenler tanımlı olduğu için print çalışacaktır
